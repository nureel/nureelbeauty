...
<div class="min-h-full flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
    <div class="bg-neutral-900 p-10 max-w-md w-full space-y-8">
      <div>
        ...
        <h2 class="mt-6 text-center text-3xl font-extrabold text-amber-400">Sahkan emel anda</h2>
        <p class="mt-2 text-center text-sm text-neutral-100">
          Sila semak emel anda atau
          <a on:click|preventDefault={send} href="/#" class="font-medium text-pink-400 hover:text-amber-400"> hantar semula kod pengesahan </a>
        </p>
      </div>
      <form class="mt-8 space-y-6" on:submit|preventDefault={submit}>
        <div class="rounded-md shadow-sm -space-y-px">
          <div>
            <label for="email-address" class="sr-only">Kod</label>
            <input bind:value={code} id="email-address" type="text" required class="appearance-none rounded-none relative block w-full px-3 py-2 border border-neutral-300 placeholder-neutral-500 text-neutral-900 focus:outline-none focus:ring-amber-400 focus:border-amber-400 focus:z-10 sm:text-sm" placeholder="Kod">
          </div>
        </div>
          <div class="{show === false ? 'hidden' : 'visible'}">
            <div class=" bg-red-400 text-neutral-100 px-4 py-3 mb-6 rounded relative" role="alert">
              <strong class="font-bold">Kod tidak sah!</strong>
